...
─────────────────────
Classify all rows from a promoted URL workbook into one of four sectors using
...
SECTOR 1: "MSP IT Services"
Covers: Managed IT support, helpdesk, remote monitoring (RMM), IT infrastructure management,
server/endpoint/device management, patch management, backup & disaster recovery, IT asset
management, cloud services (M365, Azure, AWS, Google Workspace), cloud migration, virtual
desktop (VDI), data centre management, vCIO, IT strategy & compliance, IT procurement.

SECTOR 2: "Cybersecurity Services"
Covers: Managed detection & response (MDR), SOC, threat intelligence, SIEM, endpoint
detection & response (EDR), firewall management, network security, SASE, identity & access
management (IAM/PAM), MFA, zero trust architecture, penetration testing, vulnerability
assessments, security audits, red/blue team, GRC, NIST/ISO 27001/HIPAA/SOC2/GDPR compliance.

SECTOR 3: "Telecom / Connectivity Services"
Covers: Internet connectivity (fibre, broadband), SD-WAN, WAN/LAN design, MPLS, VoIP,
UCaaS, hosted PBX, video conferencing, collaboration platforms, network cabling, structured
cabling, data centre networking, telecom equipment installation, network optimisation.

If a company does not clearly fit any of the three sectors, classify it as: "Other"

Rules:
- Return ONLY a JSON array, no explanation, no markdown, no code fences.
- Each element: {"index": <original_index>, "sector": "<sector_name>"}
- Sector must be exactly one of: "MSP IT Services", "Cybersecurity Services",
"Telecom / Connectivity Services", "Other"
...
def write_output(df: pd.DataFrame, sector_map: dict, output_path: str | None = None):
    ...
    sheets = {
        "cyber": df[df["Sector"] == "Cybersecurity Services"],
        "MSP": df[df["Sector"] == "MSP IT Services"],
        "telecom": df[df["Sector"] == "Telecom / Connectivity Services"],
